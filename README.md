...
| # | Требование | Описание |
|---|-----------|---------|
| 1 | **Next.js App Router** | Только App Router, не Pages Router |
| 2 | **Tailwind CSS** | Стили через Tailwind, без UI-библиотек (shadcn и т.п.) |
| 3 | **TypeScript** | Строгая типизация, все компоненты типизированы |
| 4 | **Адаптив** | Desktop / Tablet / Mobile — все три брейкпоинта |
| 5 | **Только вёрстка** | Без API, без логики, без state — чистая статичная вёрстка |
| 6 | **Props в компонентах** | Каждый компонент принимает props, без хардкода внутри |
| 7 | **Без UI-библиотек** | Никакого shadcn, MUI, Chakra и т.п. — всё своё |
| 8 | **Flex/Grid вёрстка** | Позиционирование через flex/grid, НЕ через `position: absolute` |
| 9 | **Pixel perfect** | 1 в 1 совпадение с Figma — каждый пиксель, цвет, текст |

> ⚠️ **Важно:** Нарушение любого из этих требований = задача не принята. Проверяй перед PR.
